...
promiseChildren :: (Monad m, MonadBaseControl IO m) => HeistT m IO (RuntimeSplice m Builder)
...
-- node as a view.
promiseChildrenWith :: (Monad n, MonadBaseControl IO n)
                    => [(Text, a -> Builder)]
                    -> Promise a
...
promiseChildrenWithTrans :: (Monad n, MonadBaseControl IO n)
                         => (b -> Builder)
                         -> [(Text, a -> b)]
                         -> Promise a
                         -> HeistT n IO (RuntimeSplice n Builder)
...
-- node as a view.
promiseChildrenWithText :: (Monad n, MonadBaseControl IO n)
                        => [(Text, a -> Text)]
                        -> Promise a
                        -> HeistT n IO (RuntimeSplice n Builder)
...
promiseChildrenWithNodes :: (Monad n, MonadBaseControl IO n)
                         => [(Text, a -> [X.Node])]
                         -> Promise a
                         -> HeistT n IO (RuntimeSplice n Builder)
...
renderTemplate :: (Monad n, MonadBaseControl IO n)
               => HeistState n
               -> ByteString
...
interpret :: (Monad m, MonadBaseControl IO m) => DList (Chunk m) -> m Builder
...
defer :: (Monad n, MonadBaseControl IO n)
      => (Promise a -> Splice n)
      -> RuntimeSplice n a
      -> Splice n
defer f getItem = do
...
-- that fit in the promise and returns a Splice that executes the promise
-- function for each item and concatenates the results.
deferMany :: (Monad n, MonadBaseControl IO n)
          => (Promise a -> Splice n)
          -> RuntimeSplice n [a]
          -> Splice n
...
-- Promise.
manyWithSplices :: (Monad n, MonadBaseControl IO n)
                => Splice n
